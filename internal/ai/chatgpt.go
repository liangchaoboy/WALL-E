...
// ChatGPTClient ChatGPT 客户端实现
type ChatGPTClient struct {
	config Config
	client *http.Client
}

// NewChatGPTClient 创建 ChatGPT 客户端
func NewChatGPTClient(config Config) (*ChatGPTClient, error) {
	if config.APIKey == "" {
		return nil, fmt.Errorf("ChatGPT API Key 不能为空")
	}
...
func (c *ChatGPTClient) GetProviderName() string {
	return "ChatGPT"
}

// ExtractNavigationIntent 从文本中提取导航意图
func (c *ChatGPTClient) ExtractNavigationIntent(ctx context.Context, text string) (*NavigationIntent, error) {
	// 构建提示词
